...
function sanitizeSource(input: Partial<LiveSource>): LiveSource | null {
  ...
  return {
    ...
    needProxy: typeof input.needProxy === 'boolean' ? input.needProxy : true,
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const newSource: LiveSource = {
      ...
      needProxy:
        typeof payload.needProxy === 'boolean' ? payload.needProxy : true,
    }

    const merged = [...sources, newSource].sort(
