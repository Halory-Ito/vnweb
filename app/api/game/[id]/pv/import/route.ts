...
const importPv = async (
  ...
  try {
    ...
    if (!file.type.startsWith('video/')) {
      return NextResponse.json({ error: 'PV仅支持视频文件' }, { status: 400 })
    }

    if (file.size <= 0 || file.size > MAX_UPLOAD_SIZE) {
      return NextResponse.json(
        { error: '文件大小需在 1B 到 200MB 之间' },
        { status: 400 },
      )
    }
    ...
    return NextResponse.json({
      data: {
        ...
        path: `/assets/pv/${gameId}/${fileName}`,
      },
    })
  } catch (error) {
    const message = (error as Error).message || '导入PV失败'
