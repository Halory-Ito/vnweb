        ...
        for (const entry of entries) {
            if (entry.isDirectory()) {
                try {
                    ...
                    const idMatch = manifestContent.match(
                        /id:\s*["']([^"']+)["']/,
                    );
                    const nameMatch = manifestContent.match(
                        /name:\s*["']([^"']+)["']/,
                    );
                    ...
                    const iconMatch = manifestContent.match(
                        /icon:\s*["']([^"']+)["']/,
                    );
                    ...
                    if (idMatch && nameMatch) {
                        ...
                        const authorsValue = authorsBlockMatch
                            ? authorsBlockMatch[1]
                            : "";
                        const authors = authorsValue
                            ? [...authorsValue.matchAll(/["']([^"']+)["']/g)]
                                .map(
                                    (match) => match[1],
                                )
                            : [];
