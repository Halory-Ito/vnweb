...
export async function POST(_req: NextRequest) {
    try {
        const rows = await db
            ...
            .innerJoin(
                GameInfoTable,
                eq(GameRecordTable.gameId, GameInfoTable.id),
            )
            .where(
                and(
                    gt(GameRecordTable.playTime, 0),
                    gt(GameRecordTable.gameId, 0),
                ),
            )
            .orderBy(
        ...
        for (const [, records] of grouped) {
            ...
            for (const record of records) {
                ...
                const end = startDate.isValid()
                    ? startDate.add(durationSeconds, "second").format(
                        "YYYY-MM-DD HH:mm:ss",
                    )
                    : "";
            ...
            if (coverPath && imageExtension) {
                ...
                if (imageWidth > 0 && imageHeight > 0) {
                    ...
                    const containerWidthPx =
                        excelColWidthToPx(COVER_COL_WIDTH) -
                    ...
                    const renderWidthPx = Math.max(
                        1,
                        Math.floor(imageWidth * scale),
                    );
                    const renderHeightPx = Math.max(
                        1,
                        Math.floor(imageHeight * scale),
                    );
                    ...
                    const colOffset = offsetXPx /
                        excelColWidthToPx(COVER_COL_WIDTH);
                    const rowOffset = offsetYPx /
                        excelRowHeightToPx(COVER_ROW_HEIGHT);
