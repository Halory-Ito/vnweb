...
const syncSteamPlaytime = async (req: NextRequest) => {
  try {
    ...
    const ownedGames = await fetchOwnedGames(steamId, apiKey, proxySettings)
    ...
    for (const game of ownedGames) {
      ...
      const playtimeSeconds = Math.floor((game.playtime_forever ?? 0) * 60)
    ...
    return NextResponse.json({
      data: {
        success: true,
        message: `成功同步 ${updatedCount} 个游戏的时长，新增 ${recordedCount} 条计时记录`,
      },
    })
  } catch (error) {
