...
import { getAlbumDetails, searchAlbums } from './utils'
...
import type {
  GameOption,
  KhinsiderAlbum,
  KhinsiderAlbumDetails,
  OstItem,
} from './types'

type OstFormDialogProps = {
  open: boolean
  editingItem: OstItem | null
...
export function OstFormDialog({
  ...
  const handleSearch = async () => {
    ...
    setIsSearching(true)
    try {
      const results = await searchAlbums(kw)
  ...
  const handleSelectAlbum = async (album: KhinsiderAlbum) => {
    setShowSearchResultsDialog(false)
    setIsLoadingAlbum(true)
    try {
      const details = await getAlbumDetails(album.url)
      setAlbumDetails(details)
      setForm((prev) => ({
        ...prev,
        name: details.name,
        // 去掉 thumbs/ 前缀获取原图，如果没有封面则使用搜索结果中的封面，最后才是空字符串
        cover: details.covers[0].replace('thumbs/', '') || album.cover || '',
      }))
    } catch (error) {
  ...
  const handleSelectSong = (
    ...
    setForm((prev) => ({ ...prev, name: song.name, cover: albumCover }))
  }
  ...
  const handleSubmit = () => {
    ...
    // resource 只用于标注音源，歌曲列表保存到 game_ost_songs 表
    const resource = 'khinsider'
              ...
              <TableBody>
                {searchResults.map((album) => (
                  <TableRow key={album.url}>
                    ...
                    <TableCell>
                      {/* 核心修改：设置 max-width 和 truncate */}
                      <div
                        className="max-w-100 truncate text-sm font-medium"
                        title={album.name} // 鼠标悬停显示全名
                      >
                        {album.name}
                      </div>
                    </TableCell>
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="flex max-h-[85vh] max-w-3xl flex-col overflow-hidden">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingItem ? '修改 OST 信息' : '从 Khinsider 搜索并添加 OST'}
            </DialogDescription>
          </DialogHeader>
                ...
                {albumDetails && (
                  <div className="space-y-4">
                    ...
                    <div className="bg-muted/50 flex items-center gap-3 rounded-lg border p-3">
                      {albumDetails.covers[0] ? (
                        <img
                          src={albumDetails.covers[0]}
                    ...
                    <div className="bg-muted/50 max-h-72 overflow-y-auto rounded-lg border">
                      {albumDetails.songs.map((song, idx) => (
                        <button
                          ...
                          onClick={() =>
                            handleSelectSong(song, albumDetails.covers[0] || '')
                          }
                        >
