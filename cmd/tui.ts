...
const printHelp = () => {
    console.log("vnweb 命令说明:");
    console.log("  vnweb                         启动服务器");
    console.log("  vnweb list game               列出所有游戏");
    console.log("  vnweb list collection         列出所有收藏夹");
    console.log("  vnweb search <keyword>        根据关键字搜索游戏");
    console.log("  vnweb open <collection_name>  列出指定收藏夹中的游戏");
    console.log("  vnweb start <game_id>         启动游戏并进行计时");
};
...
const main = async () => {
    ...
    if (cmd === "list" && sub === "game") {
        await listGames();
        return;
    }

    if (cmd === "list" && sub === "collection") {
        await listCollections();
        return;
    }

    if (cmd === "search") {
        await searchGames([sub, ...rest].filter(Boolean).join(" "));
        return;
    }

    if (cmd === "open") {
        await openCollection([sub, ...rest].filter(Boolean).join(" "));
        return;
    }
