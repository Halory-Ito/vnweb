...
const normalizeInfoboxValue = (value: BgmCharacterInfoboxItem["value"]) => {
  ...
  if (Array.isArray(value)) {
    return value.map((item) => item?.v || "").filter(Boolean).join("/");
  }
  return "";
};
...
const mapBangumiBloodTypeFromText = (value: string) => {
  ...
  if (
    normalized === "A" || normalized === "B" || normalized === "AB" ||
    normalized === "O"
  ) {
    return normalized;
  }
...
const pickBangumiNameFromInfobox = (
  ...
  const match = infobox.find((item) =>
    (item.key || "").trim().toLowerCase() === "简体中文名"
  );
...
const mapBangumiDetailToCharacterFields = (detail: BgmCharacterDetail) => {
  ...
  const mappedGenderTuple = mapBangumiGenderToTuple(detail.gender) ||
    mapBangumiGenderToTuple(infoboxGender);
  const mappedBloodType = mapBangumiBloodType(detail.blood_type) ||
...
const fetchBangumiCharacters = async (
  ...
  return Promise.all(
    rows.map(async (item, index) => {
      ...
      let detailMapped:
        | ReturnType<typeof mapBangumiDetailToCharacterFields>
        | null = null;
      if (
        typeof bgmCharacterId === "number" && Number.isFinite(bgmCharacterId)
      ) {
        try {
      ...
      const imageUrl = detailMapped?.imageUrl ||
...
export const syncVndbCharactersByGameId = async (
  ...
  const vndbRows = needVndb && vnId
    ? await fetchVndbCharacters(gameId, vnId, now, saveImagesToLocal)
    : [];
  const bgmRows = needBangumi && bgmSubjectId
